...
use gitly_app::GitlyWorkspace;

#[derive(Debug, Parser)]
#[command(name = "gitly")]
#[command(about = "Gitly customer workspace binary")]
struct Cli {
    #[arg(long)]
...
fn main() -> Result<()> {
    ...
    let workspace = match cli.app_root {
        Some(path) => GitlyWorkspace::at(path)?,
        None => GitlyWorkspace::default()?,
    };
...
fn describe(workspace: &GitlyWorkspace, config_path: &PathBuf) -> Result<()> {
    let summary = workspace.describe(config_path)?;
    println!("Gitly customer workspace");
...
fn validate(workspace: &GitlyWorkspace, config_path: &PathBuf) -> Result<()> {
    let validation = workspace.validate(config_path)?;
    println!("Gitly validation passed");
...
fn assets(workspace: &GitlyWorkspace, config_path: &PathBuf, command: AssetsCommand) -> Result<()> {
    match command {
        AssetsCommand::Publish => {
            let publication = workspace.publish_assets(config_path)?;
            println!("Gitly asset publication");
...
fn migrate(workspace: &GitlyWorkspace, config_path: &PathBuf, command: MigrateCommand) -> Result<()> {
    match command {
        MigrateCommand::Apply { dry_run, yes } => {
            if !dry_run && !yes {
                bail!("`gitly migrate apply` requires `--yes` unless `--dry-run` is used");
            }
            let report = workspace.migrate_apply(config_path, dry_run)?;
            println!("Gitly migration apply");
...
fn serve(workspace: &GitlyWorkspace, config_path: &PathBuf, bind: Option<String>) -> Result<()> {
    let bootstrap = workspace.build_bootstrap(config_path)?;
    bootstrap.serve_from_env(bind)
}

fn up(workspace: &GitlyWorkspace, config_path: &PathBuf, bind: Option<String>) -> Result<()> {
    let validation = workspace.validate(config_path)?;
    println!(
...
fn extension_checksums(workspace: &GitlyWorkspace) -> Result<()> {
    println!(
        "gitly-community-pulse {}",
        gitly_app::gitly_community_pulse_demo_sha256(workspace.app_root())?
    );
    println!(
        "gitly-actions-scheduler {}",
        gitly_app::gitly_actions_scheduler_demo_sha256(workspace.app_root())?
    );
    Ok(())
}
...
fn linked_backend(command: LinkedBackendCommand) -> Result<()> {
    match command {
        LinkedBackendCommand::Describe => {
            let summary = gitly_backend::linked_plugin_summary();
            println!("Gitly linked backend");
        ...
        LinkedBackendCommand::Repository => {
            println!("{:#?}", gitly_backend::repository());
        }
        LinkedBackendCommand::Pulls => {
            println!("{:#?}", gitly_backend::pull_requests());
        }
        LinkedBackendCommand::Workflows => {
            println!("{:#?}", gitly_backend::workflow_runs());
        }
        LinkedBackendCommand::Organization => {
            println!("{:#?}", gitly_backend::organization());
        }
        LinkedBackendCommand::User => {
            println!("{:#?}", gitly_backend::user());
        }
    }
    Ok(())
}
