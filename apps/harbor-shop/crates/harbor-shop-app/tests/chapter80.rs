...
use harbor_shop_app::{HarborShopWorkspace, harbor_waitlist_tools_demo_sha256};
...
fn harbor_readme_and_manifest_make_runtime_installed_wasm_concrete() {
    ...
    let package = include_str!("../../../extensions/harbor-waitlist-tools/package.toml");
    let source_wat =
        include_str!("../../../extensions/harbor-waitlist-tools/harbor-waitlist-tools.wat");
...
fn harbor_waitlist_tools_declares_the_real_demo_checksum() {
    let app_root = HarborShopWorkspace::default()
        .unwrap()
        .app_root()
        .to_path_buf();
    let expected = harbor_waitlist_tools_demo_sha256(&app_root).unwrap();
    let package = include_str!("../../../extensions/harbor-waitlist-tools/package.toml");
    let app_manifest = include_str!("../../../app.toml");
...
"#,
    ...
    assert!(body.contains("Harbor Waitlist Tools is active"), "{body}");
}
