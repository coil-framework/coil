...
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct OctohubOrganization {
    pub handle: String,
    pub name: String,
    pub members: u32,
...
pub struct OctohubLinkedPluginSummary {
    pub id: String,
    pub display_name: String,
    pub version: String,
...
pub struct OctohubBackend;

pub fn plugin() -> OctohubBackend {
    OctohubBackend
}

pub fn repository() -> OctohubRepository {
    OctohubRepository {
...
pub fn pull_requests() -> Vec<OctohubPullRequest> {
    vec![
        OctohubPullRequest {
            number: 184,
...
pub fn workflow_runs() -> Vec<OctohubWorkflowRun> {
    vec![
        OctohubWorkflowRun {
...
pub fn organization() -> OctohubOrganization {
    OctohubOrganization {
...
pub fn linked_plugin_summary() -> OctohubLinkedPluginSummary {
    let plugin = plugin();
    let descriptor = plugin.descriptor();
    OctohubLinkedPluginSummary {
        id: descriptor.id,
...
impl CustomerBackendPlugin for OctohubBackend {
    fn descriptor(&self) -> CustomerPluginDescriptor {
        CustomerPluginDescriptor::new(
            "octohub-backend",
            "OctoHub Linked Backend",
            env!("CARGO_PKG_VERSION"),
        )
        .with_documentation_url("apps/octohub/README.md")
    }
...
impl CmsHooks for OctohubBackend {
    fn validate_page_publish(
            ...
            && !draft.body_html.to_ascii_lowercase().contains("accessibility")
        {
            return Ok(CmsPublishDecision::reject(
                "octohub.cms.readme.accessibility_required",
        ...
        audit.record(
            davenda_customer_sdk::AuditEntry::new(
                "octohub.cms.publish.validated",
                "cms.page",
                draft.page_id.clone(),
                "allowed",
            )
            .with_detail(format!("Validated `{}` for OctoHub publishing policy.", draft.slug)),
        )?;
...
mod tests {
    ...
    fn linked_backend_descriptor_is_stable() {
        ...
        assert_eq!(descriptor.id, "octohub-backend");
        assert_eq!(descriptor.display_name, "OctoHub Linked Backend");
    }
    ...
    fn linked_backend_summary_reports_cms_hook_registration() {
        ...
        assert_eq!(summary.id, "octohub-backend");
        assert_eq!(
            summary.hook_kinds,
