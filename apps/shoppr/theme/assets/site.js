...
%o`, message, error2, detail);
  ...
  var SiteInteractiveController = class extends Controller {
    constructor() {
      super(...arguments);
      __publicField(this, "carouselIntervalId", null);
    }
    connect() {
    ...
    setupPanelToggles() {
      this.element.querySelectorAll("[data-panel-toggle]").forEach((button) => {
    ...
    setupCarousel() {
      const carousel = this.element.querySelector("[data-carousel='hero']");
      if (!carousel) return;
      ...
      if (!window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
        this.carouselIntervalId = window.setInterval(() => show(index + 1), 7e3);
      }
    }
    setupAccordions() {
      this.element.querySelectorAll(".accordion-trigger").forEach((button) => {
        button.addEventListener("click", () => {
    ...
    setupSizePicker() {
      this.element.querySelectorAll(".size-picker").forEach((picker) => {
    ...
    setupGallery() {
      this.element.querySelectorAll(".product-gallery").forEach((gallery) => {
