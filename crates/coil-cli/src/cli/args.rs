...
use coil_auth::{Capability, DefaultSubject, Entity, ExplainOptions, Namespace, Relation};
...
pub(crate) fn parse(args: impl IntoIterator<Item = String>) -> Result<CliInput, CliRunError> {
    let mut config_path = std::env::var("COIL_CONFIG")
        .ok()
    ...
    match positionals.as_slice() {
        [command, subcommand] if command == "dev" && subcommand == "server" => {
            let config_path = config_path
                ...
                .ok_or_else(|| {
                    CliRunError::usage(
                        "`dev server` requires `--config <path>`, `COIL_CONFIG`, or a default config file",
                    )
                })?;
        ...
        [command, subcommand] if command == "config" && subcommand == "validate" => {
            let config_path = config_path
                ...
                .ok_or_else(|| {
                    CliRunError::usage(
                        "`config validate` requires `--config <path>`, `COIL_CONFIG`, or a default config file",
                    )
                })?;
        ...
        [command, subcommand] if command == "auth" && subcommand == "explain" => {
            let config_path = config_path
                ...
                .ok_or_else(|| {
                    CliRunError::usage(
                        "`auth explain` requires `--config <path>`, `COIL_CONFIG`, or a default config file",
                    )
                })?;
        ...
        [command, subcommand] if command == "auth" && subcommand == "check" => {
            let config_path = config_path
                ...
                .ok_or_else(|| {
                    CliRunError::usage(
                        "`auth check` requires `--config <path>`, `COIL_CONFIG`, or a default config file",
                    )
                })?;
            let subject = subject
            ...
            if command == "auth" && group == "bindings" && subcommand == "inspect" =>
        {
            let config_path = config_path
                ...
                .ok_or_else(|| {
                    CliRunError::usage(
                        "`auth bindings inspect` requires `--config <path>`, `COIL_CONFIG`, or a default config file",
                    )
                })?;
        ...
        [command, subcommand, spec_path] if command == "auth" && subcommand == "test-model" => {
            let config_path = config_path
                ...
                .ok_or_else(|| {
                    CliRunError::usage(
                        "`auth test-model` requires `--config <path>`, `COIL_CONFIG`, or a default config file",
                    )
                })?;
        ...
        [command, subcommand] if command == "auth" && subcommand == "list" => {
            let config_path = config_path
                ...
                .ok_or_else(|| {
                    CliRunError::usage(
                        "`auth list` requires `--config <path>`, `COIL_CONFIG`, or a default config file",
                    )
                })?;
            let subject = subject
        ...
        [command, subcommand] if command == "auth" && subcommand == "lookup" => {
            let config_path = config_path
                ...
                .ok_or_else(|| {
                    CliRunError::usage(
                        "`auth lookup` requires `--config <path>`, `COIL_CONFIG`, or a default config file",
                    )
                })?;
            let resource = resource.ok_or_else(|| {
            ...
            if command == "auth" && group == "package" && subcommand == "validate" =>
        {
            let config_path = config_path
                ...
                .ok_or_else(|| {
                    CliRunError::usage(
                        "`auth package validate` requires `--config <path>`, `COIL_CONFIG`, or a default config file",
                    )
                })?;
            ...
            if command == "auth" && group == "package" && subcommand == "inspect" =>
        {
            let config_path = config_path
                ...
                .ok_or_else(|| {
                    CliRunError::usage(
                        "`auth package inspect` requires `--config <path>`, `COIL_CONFIG`, or a default config file",
                    )
                })?;
        ...
        [command, subcommand] if command == "module" && subcommand == "list" => {
            let config_path = config_path
                ...
                .ok_or_else(|| {
                    CliRunError::usage(
                        "`module list` requires `--config <path>`, `COIL_CONFIG`, or a default config file",
                    )
                })?;
        ...
        [command, subcommand, module] if command == "module" && subcommand == "inspect" => {
            let config_path = config_path
                ...
                .ok_or_else(|| {
                    CliRunError::usage(
                        "`module inspect` requires `--config <path>`, `COIL_CONFIG`, or a default config file",
                    )
                })?;
        ...
        [command, subcommand, module] if command == "module" && subcommand == "install" => {
            let config_path = config_path
                ...
                .ok_or_else(|| {
                    CliRunError::usage(
                        "`module install` requires `--config <path>`, `COIL_CONFIG`, or a default config file",
                    )
                })?;
        ...
        [command, subcommand, module] if command == "module" && subcommand == "enable" => {
            let config_path = config_path
                ...
                .ok_or_else(|| {
                    CliRunError::usage(
                        "`module enable` requires `--config <path>`, `COIL_CONFIG`, or a default config file",
                    )
                })?;
        ...
        [command, subcommand, module] if command == "module" && subcommand == "disable" => {
            let config_path = config_path
                ...
                .ok_or_else(|| {
                    CliRunError::usage(
                        "`module disable` requires `--config <path>`, `COIL_CONFIG`, or a default config file",
                    )
                })?;
        ...
        [command, subcommand] if command == "migrate" && subcommand == "plan" => {
            let config_path = config_path
                ...
                .ok_or_else(|| {
                    CliRunError::usage(
                        "`migrate plan` requires `--config <path>`, `COIL_CONFIG`, or a default config file",
                    )
                })?;
        ...
        [command, subcommand] if command == "migrate" && subcommand == "apply" => {
            let config_path = config_path
                ...
                .ok_or_else(|| {
                    CliRunError::usage(
                        "`migrate apply` requires `--config <path>`, `COIL_CONFIG`, or a default config file",
                    )
                })?;
        ...
        [command, subcommand] if command == "release" && subcommand == "doctor" => {
            let config_path = config_path
                ...
                .ok_or_else(|| {
                    CliRunError::usage(
                        "`release doctor` requires `--config <path>`, `COIL_CONFIG`, or a default config file",
                    )
                })?;
        ...
        [command, subcommand] if command == "release" && subcommand == "plan" => {
            let config_path = config_path
                ...
                .ok_or_else(|| {
                    CliRunError::usage(
                        "`release plan` requires `--config <path>`, `COIL_CONFIG`, or a default config file",
                    )
                })?;
        ...
        [command, subcommand] if command == "cache" && subcommand == "warm" => {
            let config_path = config_path
                ...
                .ok_or_else(|| {
                    CliRunError::usage(
                        "`cache warm` requires `--config <path>`, `COIL_CONFIG`, or a default config file",
                    )
                })?;
        ...
        [command, subcommand] if command == "cache" && subcommand == "inspect" => {
            let config_path = config_path
                ...
                .ok_or_else(|| {
                    CliRunError::usage(
                        "`cache inspect` requires `--config <path>`, `COIL_CONFIG`, or a default config file",
                    )
                })?;
        ...
        [command, subcommand] if command == "cache" && subcommand == "invalidate" => {
            let config_path = config_path
                ...
                .ok_or_else(|| {
                    CliRunError::usage(
                        "`cache invalidate` requires `--config <path>`, `COIL_CONFIG`, or a default config file",
                    )
                })?;
        ...
        [command, subcommand] if command == "jobs" && subcommand == "status" => {
            let config_path = config_path
                ...
                .ok_or_else(|| {
                    CliRunError::usage(
                        "`jobs status` requires `--config <path>`, `COIL_CONFIG`, or a default config file",
                    )
                })?;
        ...
        [command, subcommand] if command == "jobs" && subcommand == "run" => {
            let config_path = config_path
                ...
                .ok_or_else(|| {
                    CliRunError::usage(
                        "`jobs run` requires `--config <path>`, `COIL_CONFIG`, or a default config file",
                    )
                })?;
        ...
        [command, subcommand] if command == "jobs" && subcommand == "ready" => {
            let config_path = config_path
                ...
                .ok_or_else(|| {
                    CliRunError::usage(
                        "`jobs ready` requires `--config <path>`, `COIL_CONFIG`, or a default config file",
                    )
                })?;
        ...
        [command, subcommand] if command == "jobs" && subcommand == "dead-letters" => {
            let config_path = config_path
                ...
                .ok_or_else(|| {
                    CliRunError::usage(
                        "`jobs dead-letters` requires `--config <path>`, `COIL_CONFIG`, or a default config file",
                    )
                })?;
        ...
        [command, subcommand] if command == "jobs" && subcommand == "in-flight" => {
            let config_path = config_path
                ...
                .ok_or_else(|| {
                    CliRunError::usage(
                        "`jobs in-flight` requires `--config <path>`, `COIL_CONFIG`, or a default config file",
                    )
                })?;
        ...
        [command, subcommand, dead_letter_id] if command == "jobs" && subcommand == "retry" => {
            let config_path = config_path
                ...
                .ok_or_else(|| {
                    CliRunError::usage(
                        "`jobs retry` requires `--config <path>`, `COIL_CONFIG`, or a default config file",
                    )
                })?;
        ...
        [command, subcommand] if command == "jobs" && subcommand == "promote" => {
            let config_path = config_path
                ...
                .ok_or_else(|| {
                    CliRunError::usage(
                        "`jobs promote` requires `--config <path>`, `COIL_CONFIG`, or a default config file",
                    )
                })?;
        ...
        [command, subcommand] if command == "tls" && subcommand == "status" => {
            let config_path = config_path
                ...
                .ok_or_else(|| {
                    CliRunError::usage(
                        "`tls status` requires `--config <path>`, `COIL_CONFIG`, or a default config file",
                    )
                })?;
        ...
        [command, subcommand] if command == "tls" && subcommand == "validate-challenge" => {
            let config_path = config_path
                ...
                .ok_or_else(|| {
                    CliRunError::usage(
                        "`tls validate-challenge` requires `--config <path>`, `COIL_CONFIG`, or a default config file",
                    )
                })?;
        ...
        [command, subcommand] if command == "tls" && subcommand == "renew" => {
            let config_path = config_path
                ...
                .ok_or_else(|| {
                    CliRunError::usage(
                        "`tls renew` requires `--config <path>`, `COIL_CONFIG`, or a default config file",
                    )
                })?;
        ...
        [command, subcommand] if command == "storage" && subcommand == "verify" => {
            let config_path = config_path
                ...
                .ok_or_else(|| {
                    CliRunError::usage(
                        "`storage verify` requires `--config <path>`, `COIL_CONFIG`, or a default config file",
                    )
                })?;
        ...
        [command, subcommand] if command == "storage" && subcommand == "inspect" => {
            let config_path = config_path
                ...
                .ok_or_else(|| {
                    CliRunError::usage(
                        "`storage inspect` requires `--config <path>`, `COIL_CONFIG`, or a default config file",
                    )
                })?;
        ...
        [command, subcommand] if command == "assets" && subcommand == "publish" => {
            let config_path = config_path
                ...
                .ok_or_else(|| {
                    CliRunError::usage(
                        "`assets publish` requires `--config <path>`, `COIL_CONFIG`, or a default config file",
                    )
                })?;
...
mod tests {
    ...
    fn parse_auth_explain_requires_config_and_live_subjects() {
        ...
        assert_eq!(invocation.config_path, PathBuf::from("/tmp/coil.toml"));
    ...
    fn parse_import_cutover_accepts_switch_and_rollback_inputs() {
        let input = parse([
            ...
            "coil-origin.example.net".to_string(),
            "--yes".to_string(),
        ])
        .unwrap();
    ...
    fn parse_storage_verify_accepts_policy_flag() {
        ...
        assert_eq!(config_path, PathBuf::from("/tmp/coil.toml"));
    }
