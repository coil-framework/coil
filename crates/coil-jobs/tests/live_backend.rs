use coil_config::JobBackend;
use coil_jobs::{JobsBackendAdapter, JobsModelError, JobsRuntime};
use std::path::PathBuf;

fn config(backend: JobBackend) -> coil_config::JobsConfig {
    coil_config::JobsConfig {
        backend,
        retry_limit: 3,
    }
}
