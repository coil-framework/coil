...
fn unique_dir(label: &str) -> PathBuf {
    let unique = SystemTime::now()
        ...
        .unwrap_or_default()
        .as_nanos();
    let path = std::env::temp_dir().join(format!("coil-import-{label}-{unique}"));
...
fn cutover_execution_journal_persists_non_dns_switch_targets() {
    ...
    assert_eq!(stored.traffic_targets[0].current_target, "coil-origin");
}
