...
fn require_non_empty(field: &'static str, value: String) -> Result<String, ImportModelError> {
...
fn validate_token(field: &'static str, value: String) -> Result<String, ImportModelError> {
...
mod tests {
    ...
    fn cutover_plan_surfaces_readiness_and_rollback_triggers() {
        let ready = CutoverPlan::new()
            .with_check(
                CutoverCheck::new("tls", "TLS issuance and validation are green", true, true)
                    .unwrap(),
            )
            .with_check(
                CutoverCheck::new("final-import", "Final delta import completed", true, true)
                    .unwrap(),
            );
