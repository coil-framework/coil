...
impl LiveAuthExplainBackend {
    ...
    pub(crate) fn from_config(config: &PlatformConfig) -> Result<Self, CliRunError> {
        let package =
            coil_auth::configured_auth_model_package_selection(config.auth.package.clone());
        let explainer = LiveAuthExplainHost::from_config(config, package).map_err(|error| {
            CliRunError::execution(format!(
                "failed to initialize the live auth explain backend: {error}"
            ))
...
) -> Result<coil_auth::AuthModelPackageSelection, CliRunError> {
    if config.auth.package == coil_auth::default_manifest().name {
...
"#;
    ...
    fn customer_auth_fixture() -> PathBuf {
        let suffix = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("coil-cli-auth-backend-{suffix}"));
...
"#,
    ...
    fn explanation() -> coil_auth::CapabilityExplanation {
        ...
        coil_auth::CapabilityExplanation {
    ...
    fn from_config_path_loads_checked_in_customer_auth_package() {
        ...
        assert_eq!(
            package
                ...
                .unwrap()
                .relation,
            coil_auth::Relation::FeaturedEdit
        );
        assert_eq!(
            package
                .package()
