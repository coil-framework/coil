...
use davenda_cache::{CacheBackendKind, CacheMetrics};
use davenda_jobs::JobsCoordinatorSnapshot;
use serde_json::json;
...
fn live_queue_status(state: &RuntimeServerState) -> davenda_observability::DependencyStatus {
    match live_jobs_snapshot(state) {
        Some(_) => davenda_observability::DependencyStatus::Healthy,
        None => davenda_observability::DependencyStatus::Unhealthy,
    }
}
...
fn live_object_store_status(
    ...
    match agent.request("HEAD", endpoint_url).call() {
        Ok(_) | Err(ureq::Error::Status(_, _)) => davenda_observability::DependencyStatus::Healthy,
        Err(ureq::Error::Transport(_)) => davenda_observability::DependencyStatus::Unhealthy,
    }
}
...
) -> Option<davenda_observability::MetricReading> {
    match metric_name {
        "davenda.queue.depth" => live_jobs_snapshot.map(|snapshot| {
            davenda_observability::MetricReading::Gauge(
                (snapshot.ready.len() + snapshot.scheduled.len() + snapshot.in_flight.len())
                    as i64,
            )
        }),
        "davenda.cache.hit_ratio" => live_cache_metrics.and_then(|metrics| {
...
fn prometheus_metrics_body(
    ...
    body.push_str(&format!(
        "# davenda_metrics_enabled {}\n",
        if telemetry.metrics_enabled { 1 } else { 0 }
    ));
    body.push_str(&format!(
        "# davenda_trace_enabled {}\n",
        if telemetry.trace.enabled { 1 } else { 0 }
    ));
    body.push_str(&format!(
        "# davenda_trace_sample_permyriad {}\n",
    ...
    for metric in telemetry.metrics.values() {
        ...
        match metric_reading_for_exposition(
            ...
            Some(davenda_observability::MetricReading::Counter(value)) => {
                body.push_str(&format!("# TYPE {metric_name} counter\n"));
                body.push_str(&format!("{metric_name} {value}\n"));
            }
            Some(davenda_observability::MetricReading::Gauge(value)) => {
                body.push_str(&format!("# TYPE {metric_name} gauge\n"));
                body.push_str(&format!("{metric_name} {value}\n"));
            }
            Some(davenda_observability::MetricReading::Histogram(value)) => {
    ...
    if let Some(snapshot) = live_jobs_snapshot {
        body.push_str("# TYPE davenda_runtime_jobs_ready gauge\n");
        body.push_str(&format!("davenda_runtime_jobs_ready {}\n", snapshot.ready.len()));
        body.push_str("# TYPE davenda_runtime_jobs_scheduled gauge\n");
        body.push_str(&format!(
            "davenda_runtime_jobs_scheduled {}\n",
            snapshot.scheduled.len()
        ));
        body.push_str("# TYPE davenda_runtime_jobs_in_flight gauge\n");
        body.push_str(&format!(
            "davenda_runtime_jobs_in_flight {}\n",
            snapshot.in_flight.len()
        ));
        body.push_str("# TYPE davenda_runtime_jobs_dead_letters gauge\n");
        body.push_str(&format!(
            "davenda_runtime_jobs_dead_letters {}\n",
            snapshot.dead_letters.len()
        ));
    }

    if let Some(metrics) = live_cache_metrics {
        body.push_str("# TYPE davenda_runtime_cache_hits counter\n");
        body.push_str(&format!("davenda_runtime_cache_hits {}\n", metrics.hits));
        body.push_str("# TYPE davenda_runtime_cache_stale_hits counter\n");
        body.push_str(&format!(
            "davenda_runtime_cache_stale_hits {}\n",
            metrics.stale_hits
        ));
        body.push_str("# TYPE davenda_runtime_cache_misses counter\n");
        body.push_str(&format!("davenda_runtime_cache_misses {}\n", metrics.misses));
        body.push_str("# TYPE davenda_runtime_cache_invalidations counter\n");
        body.push_str(&format!(
            "davenda_runtime_cache_invalidations {}\n",
    ...
    for trace in telemetry.recent_traces(25) {
        body.push_str(&format!(
            "# davenda_trace trace_id={} span={} outcome={} recorded_at={}\n",
