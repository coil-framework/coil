...
fn linked_customer_backend_descriptor_stays_stable() {
    let descriptor = shoppr_backend::plugin().descriptor();
...
fn admin_dashboard_surfaces_the_linked_workspace_backend() {
    ...
    assert!(
        dashboard.contains("cargo run -p shoppr -- linked-backend demo"),
        "{dashboard}"
    );

    assert!(
        app_readme.contains("cargo run -p shoppr -- describe"),
        "{app_readme}"
    );
    assert!(
    ...
    assert!(
        app_readme.contains("cargo run -p shoppr -- validate"),
        "{app_readme}"
    );
    assert!(
        app_readme.contains("cargo run -p shoppr -- migrate apply --dry-run"),
        "{app_readme}"
    );
    assert!(
        app_readme.contains("cargo run -p shoppr -- assets publish"),
        "{app_readme}"
    );
    assert!(
    ...
    assert!(
        app_readme.contains("uses only the Shoppr folder as its Docker build context"),
        "{app_readme}"
    );
    assert!(
    ...
    assert!(
        app_readme.contains("writes `apps/shoppr/.cargo/config.toml`"),
        "{app_readme}"
    );
    assert!(
        app_readme.contains("Shoppr Linked Backend"),
        "{app_readme}"
    );
    ...
    assert!(
        entrypoint.contains("exec shoppr --config \"$CONFIG_PATH\" up"),
        "{entrypoint}"
    );
    assert!(
        dockerfile.contains("cargo build --locked -p shoppr --release"),
        "{dockerfile}"
    );
    assert!(
...
fn workspace_summary_reports_real_linked_plugin_details() {
    ...
    assert_eq!(summary.linked_plugin_ids, vec!["shoppr-backend"]);
    assert_eq!(summary.linked_plugins.len(), 1);
    let plugin = &summary.linked_plugins[0];
    assert_eq!(plugin.id, "shoppr-backend");
    assert_eq!(plugin.display_name, "Shoppr Linked Backend");
    assert_eq!(
...
"#,
    ...
    assert_eq!(bootstrap.linked_plugin_ids(), vec!["shoppr-backend"]);
    assert_eq!(
    ...
    assert_eq!(plugin.plugin_id, "shoppr-backend");
    assert_eq!(plugin.display_name, "Shoppr Linked Backend");
    assert_eq!(
...
fn workspace_validate_reports_customer_owned_lifecycle_summary() {
    ...
    assert_eq!(validation.app_id, "shoppr");
    assert!(
        validation
            .module_ids
            .iter()
    ...
    assert_eq!(validation.linked_plugin_ids, vec!["shoppr-backend"]);
...
"#,
    ...
    assert_eq!(report.app_id, "shoppr");
