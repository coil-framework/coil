...
%o`, message, error2, detail);
  ...
  // theme/frontend/cms-editor.ts
  var AdminInteractiveController = class extends Controller {
    connect() {
      this.bindFilters();
      this.bindCopyButtons();
    }
    bindFilters() {
      this.element.querySelectorAll("[data-admin-filter]").forEach((scope) => {
        const input = scope.querySelector("[data-admin-filter-input]");
        if (!input) return;
        const applyFilter = () => {
          const query = input.value.trim().toLowerCase();
          scope.querySelectorAll("[data-admin-filter-item]").forEach((item) => {
    ...
    bindCopyButtons() {
      this.element.querySelectorAll("[data-copy-text]").forEach((button) => {
  ...
  var CmsEditorController = class extends Controller {
    connect() {
      this.updateSummary();
    }
    toggleBlock(event) {
    ...
    setAllCollapsed(collapsed) {
      this.element.querySelectorAll("[data-block-card]").forEach((card) => {
        card.dataset.collapsed = collapsed ? "true" : "false";
        card.classList.toggle("admin-card--collapsed", collapsed);
        const button = card.querySelector("[data-block-toggle]");
        if (button) {
    ...
    updateSummary() {
      const cards = Array.from(this.element.querySelectorAll("[data-block-card]"));
      const enabled = cards.filter((card) => card.dataset.blockEnabled === "true").length;
      const disabled = cards.length - enabled;
      const summary = this.element.querySelector("[data-cms-block-summary]");
      if (summary) {
