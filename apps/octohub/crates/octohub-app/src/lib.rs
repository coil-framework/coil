...
pub use octohub_backend::OctohubLinkedPluginSummary;

const SHOWCASE_MODULE_ID: &str = "octohub-showcase";

#[derive(Debug, Clone)]
pub struct OctohubWorkspace {
    app_root: PathBuf,
}

#[derive(Debug, Clone)]
pub struct OctohubBootstrap {
...
#[derive(Debug, Clone)]
pub struct OctohubSummary {
    pub app_root: PathBuf,
    pub config_path: PathBuf,
    pub manifest: CustomerAppManifest,
    pub config: PlatformConfig,
    pub linked_plugins: Vec<OctohubLinkedPluginSummary>,
    pub linked_plugin_ids: Vec<String>,
}

#[derive(Debug, Clone)]
pub struct OctohubLifecycleValidation {
    pub app_root: PathBuf,
    pub config_path: PathBuf,
    pub app_id: String,
...
#[derive(Debug, Clone)]
pub struct OctohubMigrationApplyReport {
    pub app_id: String,
    pub config_path: PathBuf,
    pub dry_run: bool,
...
#[derive(Debug, Clone)]
pub struct OctohubAssetPublicationReport {
    pub app_id: String,
    pub config_path: PathBuf,
...
struct OctohubShowcaseModule;

impl OctohubShowcaseModule {
...
impl PlatformModule for OctohubShowcaseModule {
    ...
    fn register(&self, registry: &mut ServiceRegistry) -> Result<(), RegistrationError> {
        registry.register_customer_app_service(
            "octohub.showcase",
            "Customer-owned OctoHub showcase routes, API, and extension surfaces",
        )
    }
}

impl OctohubWorkspace {
    ...
    pub fn at(path: impl Into<PathBuf>) -> Result<Self> {
        let path = path.into();
        if !path.exists() {
            bail!("OctoHub app root `{}` does not exist", path.display());
        }
        if !path.is_dir() {
            bail!("OctoHub app root `{}` is not a directory", path.display());
        }
    ...
    pub fn load_manifest(&self) -> Result<CustomerAppManifest> {
        ...
        let manifest = CustomerAppManifest::from_file(&manifest_path).with_context(|| {
            format!(
                "failed to load OctoHub manifest at `{}`",
    ...
    pub fn build_bootstrap(&self, config_path: impl AsRef<Path>) -> Result<OctohubBootstrap> {
        ...
        manifest
            ...
            .context("OctoHub manifest/config alignment failed")?;

        let auth_package = load_auth_model_package_at(&manifest.auth.package_name, &self.app_root)
            .with_context(|| {
                format!(
                    "failed to load OctoHub auth package `{}` from `{}`",
                    manifest.auth.package_name,
                    self.app_root.display()
                )
            })?;

        let modules = resolve_modules_from_config(&config).context("failed to resolve OctoHub modules")?;
        let extension_packages = extensions::load_extension_packages(
            &self.app_root,
            Path::new(&config.wasm.directory),
            &self.manifest_path(),
        )
        .context("failed to resolve OctoHub runtime-installed extension packages")?;
        let customer_plugins: Vec<Box<dyn CustomerBackendPlugin>> =
            vec![Box::new(octohub_backend::plugin())];
        ...
        let mut runtime_plan = manifest
            ...
            .context("OctoHub runtime build failed")?;
    ...
    pub fn validate(&self, config_path: impl AsRef<Path>) -> Result<OctohubLifecycleValidation> {
        ...
        Ok(OctohubLifecycleValidation {
            ...
            module_ids: composition
                ...
                .collect(),
            linked_plugin_ids: vec![octohub_backend::plugin().descriptor().id],
    ...
    pub fn migrate_apply(
        ...
        config_path: impl AsRef<Path>,
        dry_run: bool,
    ) -> Result<OctohubMigrationApplyReport> {
    ...
    pub fn publish_assets(
        &self,
        config_path: impl AsRef<Path>,
    ) -> Result<OctohubAssetPublicationReport> {
        let bootstrap = self.build_bootstrap(config_path)?;
        Ok(bootstrap.asset_publication_report())
    }

    pub fn describe(&self, config_path: impl AsRef<Path>) -> Result<OctohubSummary> {
        let manifest = self.load_manifest()?;
        let (config_path, config) = self.load_platform_config(config_path)?;
        manifest
            .validate_runtime_config_alignment(&config)
        ...
        let linked_plugins = vec![octohub_backend::linked_plugin_summary()];
        Ok(OctohubSummary {
            app_root: self.app_root.clone(),
            config_path,
            manifest,
...
pub fn octohub_community_pulse_demo_sha256(app_root: impl AsRef<Path>) -> Result<String> {
    extensions::compiled_demo_artifact_sha256(app_root.as_ref(), "octohub-community-pulse")
}

pub fn octohub_actions_scheduler_demo_sha256(app_root: impl AsRef<Path>) -> Result<String> {
    extensions::compiled_demo_artifact_sha256(app_root.as_ref(), "octohub-actions-scheduler")
}
...
impl OctohubBootstrap {
    pub fn server_host<R: SecretResolver>(
        ...
        self.runtime_plan
            ...
            .context("failed to build OctoHub server host")
    }
    ...
    pub fn apply_migrations(&self, dry_run: bool) -> Result<OctohubMigrationApplyReport> {
        ...
        if dry_run {
            return Ok(OctohubMigrationApplyReport {
        ...
        let tokio_runtime = tokio::runtime::Builder::new_current_thread()
            .enable_all()
            .build()
            .context("failed to start OctoHub migration runtime")?;
        ...
        let client = self
            ...
            .context("failed to connect to the OctoHub migration database")?;
        let applied_keys = tokio_runtime
            .block_on(async { client.applied_migration_keys().await })
            .context("failed to read applied OctoHub migrations")?;
        ...
        let executed_statements = if pending_steps == 0 {
            ...
            registry
                ...
                .context("failed to register OctoHub executable migrations")?;
            let batch = self
                .runtime_plan
                .runtime
                .data
                .compile_migrations(&registry)
                .context("failed to compile OctoHub executable migrations")?;
            let execution = tokio_runtime
                .block_on(async { client.apply_migrations(&batch).await })
                .context("failed to apply OctoHub migrations")?;
            execution.statements_executed
        };

        Ok(OctohubMigrationApplyReport {
            app_id: self.manifest.id.to_string(),
            config_path: self.config_path.clone(),
            dry_run: false,
            executable_steps: executable_plan.ordered_steps().len(),
    ...
    pub fn asset_publication_report(&self) -> OctohubAssetPublicationReport {
    ...
    pub fn serve_from_env(&self, bind_override: Option<String>) -> Result<()> {
        self.runtime_plan
            ...
            .context("failed to serve OctoHub from the customer runtime")
    }
...
fn validate_workspace_layout(app_root: &Path) -> Result<()> {
    if !app_root.join("app.toml").is_file() {
        bail!(
            "OctoHub manifest `{}` is missing",
            app_root.join("app.toml").display()
        );
    }
    if !app_root.join("templates").is_dir() {
        bail!(
            "OctoHub templates directory `{}` is missing",
...
fn resolve_modules_from_config(config: &PlatformConfig) -> Result<Vec<Box<dyn PlatformModule>>> {
    ...
    for module_id in &config.modules.enabled {
        if module_id == SHOWCASE_MODULE_ID {
            modules.push(Box::new(OctohubShowcaseModule));
        } else {
            modules.push(
...
fn ensure_route(runtime: &mut RuntimePlan, route: RouteDefinition) -> Result<()> {
    if runtime.http.routes.iter().any(|existing| existing.name == route.name) {
        bail!("duplicate OctoHub route `{}`", route.name);
    }
...
fn ensure_handler(runtime: &mut RuntimePlan, handler: HandlerDefinition) -> Result<()> {
    ...
    if runtime.handlers.contains_key(&route_name) {
        bail!("duplicate OctoHub handler `{route_name}`");
    }
    runtime.handlers.insert(route_name, handler);
    Ok(())
}

fn octohub_page_routes() -> Vec<(RouteDefinition, &'static str)> {
    let pages = [
        ("home", "", "octohub/home"),
    ...
    for (code, prefix) in [("en", ""), ("fr", "/fr"), ("de", "/de")] {
        for (name, path, template) in pages {
            ...
            let route = RouteDefinition::new(
                format!("octohub.{code}.{name}"),
                HttpMethod::Get,
                full_path,
            )
            .expect("static OctoHub routes should be valid");
...
fn octohub_api_routes() -> Vec<(RouteDefinition, BTreeMap<String, String>)> {
    let definitions = [
        (
            "octohub.api.repository",
            "/api/github/repository",
            octohub_backend::repo_api_payload(),
        ),
        (
            "octohub.api.pulls",
            "/api/github/pulls",
            octohub_backend::pulls_api_payload(),
        ),
        (
            "octohub.api.workflows",
            "/api/github/workflows",
            octohub_backend::workflow_api_payload(),
        ),
        (
            "octohub.api.organization",
            "/api/github/org",
            octohub_backend::organization_api_payload(),
        ),
        (
            "octohub.api.user",
            "/api/github/user",
            octohub_backend::user_api_payload(),
        ),
        (
            "octohub.api.pulse",
            "/api/github/pulse",
            BTreeMap::from([
                ...
                ("surface".to_string(), "octohub-community-pulse".to_string()),
            ]),
        ),
    ];
        ...
        .map(|(name, path, payload)| {
            (
                RouteDefinition::new(name, HttpMethod::Get, path)
                    .expect("static OctoHub API routes should be valid")
...
fn pending_migration_plan(
    ...
    for step in plan.ordered_steps() {
        ...
        pending.insert(step.clone()).with_context(|| {
            format!("failed to stage pending OctoHub migration `{}`", step.id)
        })?;
    }
    Ok(pending)
}
