...
pub(super) fn augment_manifest_with_extensions(
    ...
    manifest
        .validate()
        .context("OctoHub manifest extension configuration is invalid")?;
    Ok(manifest)
}
...
pub(super) fn load_extension_packages(
    ...
    let document = OctohubExtensionInstallDocument::from_file(manifest_path)?;
    document
        .extensions
        .into_iter()
...
struct OctohubExtensionInstallDocument {
    #[serde(default)]
    extensions: Vec<OctohubInstalledExtensionDocument>,
}

impl OctohubExtensionInstallDocument {
    fn from_file(path: &Path) -> Result<Self> {
        let input = fs::read_to_string(path).with_context(|| {
            format!(
                "failed to read OctoHub extension installs from `{}`",
                path.display()
            )
        })?;
        toml::from_str(&input).with_context(|| {
            format!(
                "failed to parse OctoHub extension installs from `{}`",
                path.display()
            )
        })
    }
...
struct OctohubInstalledExtensionDocument {
    id: String,
    package_version: String,
    artifact_sha256: String,
    customer_app_id: String,
    #[serde(default)]
    handlers: Vec<OctohubInstalledHandlerDocument>,
}

#[derive(Debug, Deserialize)]
struct OctohubInstalledHandlerDocument {
    id: String,
    #[serde(default)]
    grants: Vec<String>,
}
...
fn load_declared_extensions(manifest_path: &Path) -> Result<Vec<CustomerExtension>> {
    ...
    document
        .extensions
        .into_iter()
        .map(OctohubInstalledExtensionDocument::into_model)
        .collect()
}

impl OctohubInstalledExtensionDocument {
    fn into_model(self) -> Result<CustomerExtension> {
        let handlers = self
            .handlers
            .into_iter()
            .map(OctohubInstalledHandlerDocument::into_model)
            .collect::<Result<Vec<_>>>()?;
        let installation = ExtensionInstallation::new(self.customer_app_id, handlers)
            .context("failed to build OctoHub extension installation")?;
        ...
        .context("failed to build OctoHub customer extension")
    }
}

impl OctohubInstalledHandlerDocument {
    fn into_model(self) -> Result<HandlerInstallation> {
        Ok(HandlerInstallation::new(
            HandlerId::new(self.id).context("invalid OctoHub extension handler id")?,
...
#[derive(Debug, Deserialize)]
struct OctohubExtensionPackageDocument {
    publisher: String,
    artifact: String,
    artifact_sha256: String,
    source_wat: String,
    manifest: OctohubExtensionManifestDocument,
    #[serde(default)]
    handlers: Vec<OctohubExtensionHandlerDocument>,
}

#[derive(Debug, Deserialize)]
struct OctohubExtensionManifestDocument {
    id: String,
    display_name: String,
    version: String,
...
#[derive(Debug, Deserialize)]
struct OctohubExtensionHandlerDocument {
    id: String,
    export: String,
    point: String,
    target: String,
...
fn load_extension_package(
    ...
    let input = fs::read_to_string(&package_path).with_context(|| {
        format!(
            "failed to read OctoHub extension package `{}`",
            package_path.display()
        )
    })?;
    let document: OctohubExtensionPackageDocument = toml::from_str(&input).with_context(|| {
        format!(
            "failed to parse OctoHub extension package `{}`",
            package_path.display()
        )
    })?;
    ...
    if document.manifest.id != extension_id {
        bail!(
            "OctoHub extension package `{}` declares id `{}` instead of `{}`",
            package_path.display(),
    ...
    compile_demo_artifact(&package_dir, extension_directory, &document)
        .with_context(|| format!("failed to compile OctoHub extension artifact `{extension_id}`"))?;

    let handlers = document
        .handlers
        .into_iter()
        .map(OctohubExtensionHandlerDocument::into_model)
        .collect::<Result<Vec<_>>>()?;
    ...
    let manifest = ExtensionManifest::new(
        davenda_wasm::ExtensionId::new(document.manifest.id)
            .context("invalid OctoHub extension package id")?,
    ...
    .context("failed to build OctoHub extension manifest")?;

    ExtensionPackage::new(
        document.publisher,
        manifest,
        ExtensionArtifactSource::local_path(document.artifact)
            .context("invalid OctoHub extension artifact path")?,
        document.artifact_sha256,
        ExtensionConfigSchema::new(1, Vec::new())
            .expect("empty OctoHub extension config schema should be valid"),
    )
    .context("failed to build OctoHub extension package")
}

impl OctohubExtensionHandlerDocument {
...
fn compile_demo_artifact(
    package_dir: &Path,
    extension_directory: &Path,
    document: &OctohubExtensionPackageDocument,
) -> Result<()> {
    let Some(handler) = document.handlers.first() else {
        bail!("OctoHub extension package must declare at least one handler");
    };
    if document.handlers.len() != 1 {
        bail!("OctoHub demo extensions currently support exactly one installed handler");
    }

    let wat_path = package_dir.join(&document.source_wat);
    let wat_template = fs::read_to_string(&wat_path).with_context(|| {
        format!(
            "failed to read OctoHub extension source `{}`",
            wat_path.display()
        )
    })?;
    ...
    let artifact_bytes = match handler.point.to_ascii_lowercase().as_str() {
        ...
        other => bail!("unsupported OctoHub extension point `{other}`"),
    };
    ...
    if let Some(parent) = artifact_path.parent() {
        fs::create_dir_all(parent).with_context(|| {
            format!(
                "failed to create OctoHub extension artifact directory `{}`",
                parent.display()
            )
        })?;
    }
    fs::write(&artifact_path, artifact_bytes).with_context(|| {
        format!(
            "failed to write OctoHub extension artifact `{}`",
...
fn compile_api_artifact(wat_template: &str, export: &str) -> Result<Vec<u8>> {
    let typed_output = TypedExecutionOutput::api(
        ...
        TypedMetadata::new()
            .with_description("OctoHub community pulse API extension output")
            .expect("static OctoHub metadata should be valid"),
        None,
    )
    .expect("static OctoHub API output should be valid");
    let typed_output_bytes = typed_output
        .encode()
        .context("failed to encode OctoHub API typed output")?;
    ...
    wat::parse_str(&wat_module).context("failed to compile OctoHub API WAT")
}
...
fn compile_scheduled_job_artifact(wat_template: &str, export: &str) -> Result<Vec<u8>> {
    ...
    wat::parse_str(&wat_module).context("failed to compile OctoHub scheduled job WAT")
}
    ...
    match point.to_ascii_lowercase().as_str() {
        "api" => Ok(ExtensionPoint::Api(
            ApiExtensionPoint::new(target, [davenda_wasm::HttpMethod::Get])
                .context("invalid OctoHub API extension target")?,
        )),
        "scheduled-job" | "scheduled_job" => Ok(ExtensionPoint::ScheduledJob(
            ScheduledJobExtensionPoint::new(target, "*/30 * * * *")
                .context("invalid OctoHub scheduled-job extension target")?,
        )),
        other => bail!("unsupported OctoHub extension point `{other}`"),
    }
}
...
fn parse_grants(values: &[String]) -> Result<HostGrantSet> {
    ...
    for value in values {
        ...
        bail!("unsupported OctoHub extension grant `{normalized}`");
    }
    Ok(grants)
}
...
pub(crate) fn compiled_demo_artifact_sha256(app_root: &Path, extension_id: &str) -> Result<String> {
    ...
    let unique = SystemTime::now()
        ...
        .unwrap_or_default()
        .as_nanos();
    let extension_directory = std::env::temp_dir().join(format!("octohub-extension-{unique}"));
    ...
    let bytes = fs::read(extension_directory.join(&document.artifact)).with_context(|| {
        format!("failed to read compiled OctoHub extension artifact for `{extension_id}`")
    })?;
