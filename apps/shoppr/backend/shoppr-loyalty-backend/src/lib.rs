...
pub struct ShopprCustomerBackend;

pub fn plugin() -> ShopprCustomerBackend {
    ShopprCustomerBackend
}
...
pub fn plugin_descriptor() -> CustomerPluginDescriptor {
    ...
    .with_documentation_url("apps/shoppr/backend/shoppr-loyalty-backend/README.md")
}
...
impl CustomerBackendPlugin for ShopprCustomerBackend {
...
impl CheckoutHooks for ShopprCustomerBackend {
    fn review_order(
        &self,
        _ctx: &RequestContext,
        order: &OrderDraft,
...
impl VerifiedWebhookHooks for ShopprCustomerBackend {
    fn handle_verified_webhook(
        ...
        if webhook.source != "crm" || webhook.event != "contact-updated" {
            return Ok(WebhookHandlingResult::rejected(
                "unsupported_webhook",
                "Shoppr only handles verified CRM contact-updated webhooks in this example",
            ));
        }
...
pub fn service_overview(brand: &str) -> ServiceOverview {
    ServiceOverview {
        service: "shoppr-loyalty-backend".to_string(),
...
pub fn compute_loyalty_preview(request: &LoyaltyPreviewRequest) -> LoyaltyPreviewResponse {
    ...
    let concierge_note = if matches!(request.membership_tier, MembershipTier::Gold) && is_high_value
    {
        ...
        "Customer qualifies for Shoppr free delivery.".to_string()
    } else {
        "Standard storefront fulfillment rules apply.".to_string()
    };

    let mut tags = vec![
        "customer-app:shoppr".to_string(),
...
pub fn route_crm_contact(update: &CrmContactUpdate) -> CrmContactRoute {
    ...
    let (segment, follow_up_required, follow_up_reason) = match update.membership_tier {
        ...
        MembershipTier::Standard if update.lifecycle_stage == "winback" => (
            ...
            true,
            "Winback member should receive the Shoppr retention sequence.".to_string(),
        ),
        MembershipTier::Standard => (
            "harbor-member".to_string(),
            false,
...
mod tests {
    ...
    impl davenda_customer_sdk::RepositoryFacade for NoopRepositories {
        ...
        fn write(
            ...
            Err(BackendError::new(
                ...
                "Shoppr test noop repository does not support writes",
            ))
        }
    }
    ...
    fn request_context() -> RequestContext {
        RequestContext::new(
            CustomerAppContext::new("shoppr", "development").with_locale("en-GB"),
    ...
    fn plugin_descriptor_and_registered_hooks_stay_stable() {
        ...
        assert_eq!(descriptor.id, "shoppr-loyalty-backend");
        assert_eq!(descriptor.display_name, "Shoppr Loyalty Backend");
        assert_eq!(descriptor.version, env!("CARGO_PKG_VERSION"));
        assert_eq!(
            descriptor.documentation_url.as_deref(),
            Some("apps/shoppr/backend/shoppr-loyalty-backend/README.md")
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            OrderReviewDecision::Adjusted(
                ...
                .with_metadata_entries([
                    ...
                    (
                        "tags",
                        "customer-app:shoppr,queue:ops-manual-review,service-level:manual-clearance,ops:manual-review,shipping:international"
                    ),
                ])
            )
        );
    ...
    fn verified_webhook_hook_routes_crm_updates_through_customer_rules() {
        ...
        assert_eq!(
            ...
            WebhookHandlingResult::accepted(Some(
                "Winback member should receive the Shoppr retention sequence. [customer-app:shoppr, segment:harbor-member, lifecycle:winback]".to_string()
            ))
        );
        assert_eq!(
            audit.actions.lock().unwrap().as_slice(),
