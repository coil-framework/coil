...
WORKDIR /workspace
CMD ["cargo", "watch", "--poll", "--ignore", ".git/*", "--ignore", "target/*", "--ignore", ".coil/cache/*", "-x", "run -p {bin_package} -- up"]
"#,
...
1. Start the full local stack:
   ...
   The app service runs a debug build with `cargo watch`, so template, Rust, translation, and asset changes trigger an automatic rebuild and restart.
...
3. After the first image build, use plain `docker compose up` for the day-to-day loop.
   Cargo registry, git, and target directories are persisted through Docker volumes so the container does not redownload and rebuild the world on every restart.
...
mod tests {
    ...
    fn dev_dockerfile_uses_cargo_watch_instead_of_release_build() {
        ...
        assert!(dockerfile.contains("cargo install cargo-watch --locked"));
        assert!(dockerfile.contains("cargo\", \"watch\""));
        assert!(!dockerfile.contains("cargo build --release"));
        assert!(!dockerfile.contains("target/release"));
    }
    ...
    fn dev_compose_persists_cargo_caches_and_mounts_workspace() {
        ...
        assert!(compose.contains("- cargo-registry:/usr/local/cargo/registry"));
        assert!(compose.contains("- cargo-git:/usr/local/cargo/git"));
        assert!(compose.contains("- cargo-target:/workspace/target"));
    }
}
