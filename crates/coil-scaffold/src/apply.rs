...
fn project_readme(descriptor: &ProjectDescriptor) -> String {
    let dependency_source = match &descriptor.tooling.dependency_source {
        DependencySource::CratesIo => "Dependencies resolve from crates.io.".to_string(),
        DependencySource::Path { repo_root } => {
            format!("Dependencies resolve from the local Coil checkout at `{repo_root}`.")
        }
    };
    format!(
        r#"# {display_name}
...
2. Export the required environment:
   ...
   ```bash
   export DATABASE_URL=postgres://coil:coil@127.0.0.1:5432/{slug}
...
fn platform_config(descriptor: &ProjectDescriptor, production: bool) -> String {
    ...
    let session_store = if production { "redis" } else { "memory" };
    let l2 = if production {
        "l2 = \"redis\"\n"
    } else {
        ""
    };
