...
#[derive(Debug, Parser)]
#[command(name = "shoppr")]
#[command(about = "Shoppr customer workspace binary")]
struct Cli {
    #[arg(long)]
...
fn main() -> Result<()> {
    ...
    let workspace = match cli.app_root {
        Some(path) => ShopprWorkspace::at(path)?,
        None => ShopprWorkspace::default()?,
    };
    match cli.command {
...
fn describe(workspace: &ShopprWorkspace, config_path: &PathBuf) -> Result<()> {
    let summary = workspace.describe(config_path)?;
    println!("Shoppr customer workspace");
...
fn validate(workspace: &ShopprWorkspace, config_path: &PathBuf) -> Result<()> {
    let validation = workspace.validate(config_path)?;
    println!("Shoppr validation passed");
...
fn assets(
    ...
    match command {
        AssetsCommand::Publish => {
            ...
            println!("Shoppr asset publication");
...
fn migrate(
    ...
    match command {
        MigrateCommand::Apply { dry_run, yes } => {
            if !dry_run && !yes {
                bail!("`shoppr migrate apply` requires `--yes` unless `--dry-run` is used");
            }
            let report = workspace.migrate_apply(config_path, dry_run)?;
            println!("Shoppr migration apply");
...
fn serve(
    ...
    workspace
        ...
        .serve_from_env(bind)
        .context("Shoppr server exited with an error")
}

fn up(workspace: &ShopprWorkspace, config_path: &PathBuf, bind: Option<String>) -> Result<()> {
    let bootstrap = workspace.build_bootstrap(config_path)?;
    println!("Shoppr lifecycle bootstrap");
    ...
    bootstrap
        .serve_from_env(bind)
        .context("Shoppr lifecycle bootstrap failed while serving")
}

fn linked_backend(workspace: &ShopprWorkspace, command: LinkedBackendCommand) -> Result<()> {
    let output = match command {
        ...
        LinkedBackendCommand::LoyaltyPreview { request } => {
            ...
            let request: shoppr_backend::LoyaltyPreviewRequest =
                read_json_file(&request_path)?;
            render_json(&shoppr_backend::plugin().preview_loyalty(&request))?
        }
        LinkedBackendCommand::OrderReview { request } => {
            let request_path = linked_backend_request_path(workspace, request, "order-review.json");
            let request: shoppr_backend::OrderReviewRequest = read_json_file(&request_path)?;
            render_json(&shoppr_backend::plugin().review_checkout_order(&request))?
        }
        LinkedBackendCommand::CrmContact { request } => {
            let request_path =
                linked_backend_request_path(workspace, request, "contact-updated.json");
            let request: shoppr_backend::CrmContactUpdate = read_json_file(&request_path)?;
            render_json(&shoppr_backend::plugin().route_crm_contact_update(&request))?
        }
    };
    println!("{output}");
    Ok(())
...
fn linked_backend_describe_output() -> String {
    let summary = shoppr_backend::linked_plugin_summary();
    let hooks = summary
        .hook_kinds
        .iter()
        .map(|kind| kind.as_str())
...
fn linked_backend_demo_output(workspace: &ShopprWorkspace) -> Result<String> {
    let loyalty_request: shoppr_backend::LoyaltyPreviewRequest = read_json_file(
        &linked_backend_request_path(workspace, None, "loyalty-preview.json"),
    )?;
    let order_request: shoppr_backend::OrderReviewRequest = read_json_file(
        &linked_backend_request_path(workspace, None, "order-review.json"),
    )?;
    let crm_request: shoppr_backend::CrmContactUpdate = read_json_file(
        &linked_backend_request_path(workspace, None, "contact-updated.json"),
    )?;
    let backend = shoppr_backend::plugin();
...
fn linked_backend_request_path(
    ...
    match override_path {
        ...
        None => workspace
            .app_root()
            .join("backend/shoppr-loyalty-backend/requests")
...
mod tests {
    ...
    fn linked_backend_demo_uses_checked_in_customer_workspace_requests() {
        ...
        assert!(output.contains("Shoppr linked backend"), "{output}");
        assert!(
    ...
    fn linked_backend_request_path_defaults_to_checked_in_request_files() {
        ...
        assert!(
            path.ends_with(
                "apps/shoppr/backend/shoppr-loyalty-backend/requests/order-review.json"
            ),
            "{}",
            path.display()
        );
    ...
    fn linked_backend_describe_output_reports_registered_hook_summary() {
        ...
        assert!(
            output.contains("plugin id: shoppr-backend"),
            "{output}"
        );
        assert!(
            output.contains("display name: Shoppr Linked Backend"),
            "{output}"
        );
        assert!(
            output.contains("registered hooks: checkout, verified_webhook"),
        ...
        assert!(
            output.contains("documentation: apps/shoppr/backend/README.md"),
            "{output}"
        );
    }
    ...
    fn cli_accepts_customer_owned_lifecycle_commands() {
        ...
        let cli = Cli::try_parse_from(["shoppr", "assets", "publish"])
        ...
        let cli = Cli::try_parse_from(["shoppr", "validate"])
            .expect("validate command should parse");
        assert!(matches!(cli.command, Command::Validate));

        let cli = Cli::try_parse_from(["shoppr", "up"])
